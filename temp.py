...
def add_line(path, color="blue"):
    folium.PolyLine(locations=[list(reversed(
        coord)) for coord in path['features'][0]['geometry']['coordinates']], color=color).add_to(m)
    return
...
m = folium.Map(location=list(
    reversed([72.8429, 19.0194])), tiles="cartodbpositron", zoom_start=13)
...
add_marker(cords=user_cords[0],
           message=f'User-start {user_cords[0]}', color="orange")
add_marker(cords=user_cords[1],
           message=f'User-end {user_cords[1]}', color="purple")
...
for i in range(len(coords)):
    ...
    add_line(path=route)
    add_marker(cords=coords[i][0],
               message=f'Route-start {coords[i][0]}', color="green")
    add_marker(cords=coords[i][1],
               message=f'Route-end {coords[i][1]}', color="red")
    ...
    des_route_end = client.directions(coordinates=[user_cords[1], route_steps[-1]],
                                      profile='foot-walking',
                                      format='geojson')
    print(src_route_start["features"][0]["properties"]["segments"][0]["distance"],
          des_route_end["features"][0]["properties"]["segments"][0]["distance"])
...
add_marker(cords=min_src_cord,
           message=f"Minsrc- {min_src_cord}", color="black")
add_marker(cords=min_des_cord,
           message=f"Mindes- {min_des_cord}", color="black")
